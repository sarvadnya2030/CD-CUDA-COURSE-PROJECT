...
class BenchmarkResult:
    ...
    roofline / occupancy / graph fields are filled by their respective analyzers
    and default to None when that analysis was not run.
    """
    # ── Identity ─────────────────────────────────────────────────────────
    kernel: str
    variant: str
    ...
    # ── Correctness (filled by CorrectnessVerifier) ───────────────────────
...
# ── Nsight Compute integration (optional) ─────────────────────────────────

def collect_ncu_metrics(binary: Path, metrics: Optional[List[str]] = None) -> dict:
    """
    ...
    cmd = ["ncu", "--metrics", metric_str, "--csv",
           "--target-processes", "all", str(binary)]
    try:
